...
function json2(body, status = 200) {
  return new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json", ...CORS2 } });
}
__name(json2, "json");
...
async function onRequestGet2({ env }) {
  try {
    ...
    return json2({ error: err.message }, 500);
  }
}
...
async function onRequestPut2({ request, env }) {
  try {
    ...
    if (!name || typeof plan !== "object") {
      return json2({ error: "Missing name or plan" }, 400);
    }
...
// ../.wrangler/tmp/pages-GNEIhh/functionsRoutes-0.46699378076314657.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-naP2rB/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-naP2rB/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.03525307853066206.mjs.map
